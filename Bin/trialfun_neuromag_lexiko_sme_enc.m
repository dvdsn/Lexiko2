function [trl] = trialfun_neuromag_lexiko_sme_enc(cfg)
...
for k=45:length(event)-6
  if strcmp(event(k).type, 'STI101')
      ...
      % Define our current trigger and get the item id
      p = event(k).value;
      p = p - rem(p, 10);
 
      % Define our search array
      studyitems = [event((k-40):(k-2)).value]; 

      % Find the encoding word corresponding to it, with ismember
      targetitems = cfg.trialdef.stmvalue;
      target = targetitems(find((targetitems - rem(targetitems,10)) == p));
      studyitemindex = find(ismember(studyitems, target),1);
      idx = k-40+studyitemindex-1;
      
      % add the probe word time index to the trl definition
      if idx>0,
      encodingsample = event(idx).sample;
      begsample = encodingsample - cfg.trialdef.prestim*hdr.Fs;
      endsample = encodingsample + cfg.trialdef.poststim*hdr.Fs - 1;
      offset = -1.*cfg.trialdef.prestim*hdr.Fs;  
      
      % trl(end+1, :) = round([begsample endsample offset]);
      trl(end+1, :) = round([begsample endsample offset event(idx).value event(k).value ...
          cfg.trialdef.respvalue event(k+1).value retrievalblock]);      
