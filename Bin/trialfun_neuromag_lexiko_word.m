function [trl] = trialfun_neuromag_lexiko_word(cfg)
...
for k=2:length(event)-1
  if strcmp(event(k).type, 'STI101')
      ...
      begsample = event(k).sample - cfg.trialdef.prestim*hdr.Fs;
      endsample = event(k).sample + cfg.trialdef.poststim*hdr.Fs - 1;
      offset = - cfg.trialdef.prestim*hdr.Fs;  
      
      trl(end+1, :) = round([begsample endsample offset event(k).value ...
          event(k+1).value retrievalblock]);
